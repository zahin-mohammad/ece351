...
public class TestSimplifierNotExpr extends BaseTest351 {
	...
	@Test
	public void testTripleNegative() {
		final VarExpr x = new VarExpr("x");
		final NotExpr e = new NotExpr(new NotExpr(new NotExpr(x)));
		assertTrue(e.repOk());
		final Expr result = e.simplify();
		assertTrue(result.repOk());
		assertEquals(new NotExpr(x), result);
	}
	@Test
	public void testQuadrupleNegative() {
		final VarExpr x = new VarExpr("x");
		final NotExpr e = new NotExpr(new NotExpr(new NotExpr(new NotExpr(x))));
		assertTrue(e.repOk());
		final Expr result = e.simplify();
		assertTrue(result.repOk());
		assertEquals(x, result);
