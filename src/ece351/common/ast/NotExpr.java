...
public final class NotExpr extends UnaryExpr{
    ...
    protected final Expr simplifyOnce() {		
    	// simplify our child first
    		...
    		// nothing changed
    		// something changed
		Expr simplifiedChild;
		Boolean doubleNot = false;
		if (this.getClass() == expr.getClass()){
			simplifiedChild = ((NotExpr)this.expr).expr.simplifyOnce();
			doubleNot = true;
		}else{
			simplifiedChild = this.expr.simplifyOnce();
		}

		if (doubleNot){
			if (simplifiedChild.getClass() == ConstantExpr.class){
				return ConstantExpr.make(((ConstantExpr)simplifiedChild).b);
			}
			...
			return new NotExpr(simplifiedChild);
		}
    }
